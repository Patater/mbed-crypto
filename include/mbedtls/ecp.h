 ...
 *                  they may return #MBEDTLS_ERR_ECP_IN_PROGRESS. For functions
 *                  that accept a "restart context" argument, passing NULL
 *                  disables restart and makes the function equivalent to the
 *                  function with the same name with \c _restartable removed.
 *                  For functions in the ECDH module, restart is disabled
 *                  unless the function accepts an "ECDH context" argument and
 *                  mbedtls_ecdh_enable_restart() was previously called on that
 *                  context. For function in the SSL module, restart is only
 *                  enabled for specific sides and key exchanges (currently
 *                  only for clients and ECDHE-ECDSA).
 *
