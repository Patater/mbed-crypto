 ...
 *   #PSA_KEY_TYPE_IS_ECC_PUBLIC_KEY is true), the format is the the format is
 *   the uncompressed representation defined by SEC1 &sect;2.3.3 as the content
 *   of an ECPoint. Let `m` be the bit size associated with the curve, i.e. the
 ...
 *                          peer_key, peer_key_length)` where private_key_type
 *                          is the type of private_key. For example, for EC
 *                          keys, this means that peer_key is interpreted as a
 *                          point on the curve that the private key is on. The
